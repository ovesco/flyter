...
This example demonstrates how to build a basic and simple TailwindCSS theme.
It is not included in the library because Tailwind is, by itself, highly configurable.
Use it to match your design and change the classes.

```ts
import PopupRenderer from "../src/renderer/PopupRenderer";
import flyter, { Theme, PopupRenderer } from "../src/Theme";

type TailwindThemeConfig = {
  // Add custom required configuration values to your theme here
  arrowSize: number;
}

const TailwindTheme = (config: TailwindThemeConfig): Theme => ({
  /**
   * These class are used to style default input types to cover the basic types
   * included with Flyter. You can override these classes to match your own
   */
  types: {
    text: {
      class:
      ...
      onInit(renderer: any) {
        ...
        const updatePlacement = (placement: string) => {
          ...
          if (arrow) {
            ...
            if (placement === "bottom") {
              className +=
                ` border-b-[${config.arrowSize}px] border-b-slate-200 dark:border-t-slate-700 -mt-[${config.arrowSize}px]`;
            } else if (placement === "top") {
              className +=
                ` border-t-[${config.arrowSize}px] border-t-slate-200 dark:border-t-slate-700 top-full -mt-1`;
            }
...
flyter.registerTheme("tailwind", TailwindTheme, { arrowSize: 8 });
```