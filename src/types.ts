...
export type TypeConstructor = new (getSession: () => EditionSession, config: anyConfigObject) => FlyterType<anyConfigObject>;
export type RendererConstructor = new (getSession: () => EditionSession, config: anyConfigObject) => FlyterRenderer<anyConfigObject>;

export type rendererData = { rendererConfig: anyConfigObject, renderer: RendererConstructor };
export type typeData = { typeConfig: anyConfigObject, type: TypeConstructor };

export abstract class FlyterType<Config> {

  constructor(protected getSession: () => EditionSession, protected config: Config) {}
