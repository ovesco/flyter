...
type valOrAsync<T> = T | ((renderer: InlineRenderer) => T) | ((renderer: InlineRenderer) => Promise<T>);
...
class InlineRenderer extends FlyterRenderer<InlineConfigType> {
  ...
  async init() {
    this.markup = parseTemplate(await promisify(resolveAsync(this.config.inlineTemplate, this)));
    this.closeOnClickOutside = await(promisify(resolveAsync(this.config.closeOnClickOutside, this)));
    const containerClass = await promisify(resolveAsync(this.config.containerClass, this));
    if (containerClass.trim() !== '' && containerClass !== null) {
      this.markup.classList.add(containerClass);
    }
  ...
  async show(markup: HTMLElement) {
    ...
    this.getSession().getInstance().getDomTarget().append(this.markup as HTMLElement);
