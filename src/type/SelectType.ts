...
export const baseSelectConfig: SelectTypeConfig = {
  dataSource: [],
  class: "",
  multiple: false,
  displaySeparator: ",",
...
class SelectType extends BaseChoiceType<SelectTypeConfig, HTMLSelectElement> {
  setValue(value: any) {
    const values = !Array.isArray(value) ? [value] : value;
    values.forEach((val) => {
      const option = Array.from(this.markup.options).find(
        (it) => `${it.value}` === `${val}`
      );
  ...
  getCurrentValue() {
    const values = Array.from(this.markup.options)
  ...
  getTemplate() {
    return `
      <select class="${this.config.class}" ${
      this.config.multiple ? "multiple" : ""
    } ${ATTR_DISABLABLE}>
        ${
          this.config.showEmptyValue
