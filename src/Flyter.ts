...
import { RendererConstructor, TypeConstructor, anyConfigObject, DeepPartial } from "./types";
...
class Flyter {
  ...
  private renderers: Map<string, { baseRendererConfig: anyConfigObject, renderer: RendererConstructor }> = new Map();

  private types: Map<string, { baseTypeConfig: anyConfigObject, type: TypeConstructor }> = new Map();

  private themes: Map<string, { order: number, baseThemeConfig: anyConfigObject, theme: ThemeProvider, name: string }> = new Map();

  registerRenderer(name: string, renderer: RendererConstructor, baseRendererConfig: anyConfigObject) {
    this.renderers.set(name, { baseRendererConfig, renderer });
  }

  registerType(name: string, type: TypeConstructor, baseTypeConfig: anyConfigObject) {
    this.types.set(name, { baseTypeConfig, type });
  }

  registerTheme(name: string, theme: ThemeProvider, baseThemeConfig: anyConfigObject) {
  ...
  private attachMultiple(nodeElements: HTMLElement[] | NodeList, config: DeepPartial<Config>) {
    const elements = [...nodeElements] as HTMLElement[];
  ...
  private getType(name: string, themeConfig: anyConfigObject) {
    ...
    const { baseTypeConfig, type } = this.types.get(name);
  ...
  private getRenderer(name: string, themeConfig: anyConfigObject) {
    ...
    const { baseRendererConfig, renderer } = this.renderers.get(name);
  ...
  private resolveThemeConfig(themesData: {[ key: string ]: anyConfigObject }): Theme {
    ...
    const themesToLoad = themeNames.map((name) => this.themes.get(name)).sort((a, b) => a.order > b.order ? 1 : -1);
